...
static void http_server_fw_update_reset_timer(void)
{
    if (g_fw_update_status == OTA_UPDATE_SUCCESSFUL)
    {
        ...
        // 웹페이지에 승인을 다시 받고 타이머를 초기화할 수 있는 기회를 제공합니다.
...
static esp_err_t http_server_OTA_update_handler(httpd_req_t *req)
{
    ...
    do
    {
        ...
        // httpd_req_recv 함수는 정상적으로 종료되면 0을 반환
        ...
        // 이번이 우리가 수신하는 첫 번째 데이터입니까?
        // 그렇다면 헤더에 필요한 정보가 있을 것입니다.
        if (!is_req_body_started)
        {
            is_req_body_started = true;

            // .bin 파일 콘텐츠의 위치 가져오기(웹 양식 데이터 제거)
            // char *body_start_p = strstr(ota_buff, "\r\n\r\n") + 4;
            char *body_start_p = strstr(ota_buff, "\r\n\r\n") + strlen("\r\n\r\n");
            // strstr() 함수는 string1에서 string2의 첫 번째 표시를 찾습니다.
    ...
    if (esp_ota_end(ota_handle) == ESP_OK)
    {
        ...
        if (esp_ota_set_boot_partition(update_partition) == ESP_OK)
        {
            ...
            ESP_LOGI(TAG, "http_server_OTA_update_handler: Next boot partition subtype %d at offset 0x%x",
    ...
    // 파일 전체에서 전역 변수를 업데이트하지 않으므로 상태에 대한 메시지를 보냅니다.
    if (flash_successful)
    {
...
/**
 * OTA status handler는 OTA 업데이트가 시작된 후 펌웨어 업데이트 상태로 응답하고
 * 페이지가 처음 요청될 때 컴파일 시간/날짜로 응답합니다.
 * @param req HTTP request for which the uri needs to be handled
 * @return ESP_OK
 */
